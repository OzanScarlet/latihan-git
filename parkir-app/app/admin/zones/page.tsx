        ...
        <CardContent>
          <form action={createZone} className="flex flex-wrap items-end gap-3">
            ...
            <div>
              ...
              <input name="capacity" type="number" min={1} defaultValue={10} required className="h-10 w-24 rounded-lg border border-slate-300 px-3 text-sm focus:border-slate-900 focus:outline-none" />
            </div>
      ...
      {zones.length === 0 ? (
        ...
        zones.map((z) => (
          <Card key={z.id}>
            ...
            <CardContent>
              {/* Add slot */}
              <form action={createSlot} className="flex flex-wrap items-end gap-3 mb-4">
                <input type="hidden" name="zoneId" value={z.id} />
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Kode slot</label>
                  <input name="code" required placeholder="A-07" className="h-10 rounded-lg border border-slate-300 px-3 text-sm focus:border-slate-900 focus:outline-none" />
                </div>
                <div>
                ...
                <button className="h-10 rounded-lg border border-slate-300 px-4 text-sm font-medium hover:bg-slate-100">Tambah slot</button>
              </form>
